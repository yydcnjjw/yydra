...
fn distribution_ships_both_complete_license_texts_and_exact_spdx_markers() {
    ...
    for package in [
        ...
        "crates/yydra-cli/template/product-workspace/frontend/modules/yydra-client-settings",
    ] {
        assert_eq!(
            root_mit,
            fs::read(repository.join(package).join("LICENSE-MIT")).unwrap()
        );
        assert_eq!(
            root_apache,
            fs::read(repository.join(package).join("LICENSE-APACHE")).unwrap()
        );
    }
